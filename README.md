# Paint3 —— 用户操作手册（README）

> 本程序是《计算机图形学》课程实验 1~4（含部分挑战）在同一框架中的实现，支持**基本图元绘制、区域填充、线段/多边形裁剪、二维几何变换、Bezier 与 B-样条曲线**等功能。程序以 MFC 对话框为界面，通过“模式（Mode）+ 算法（Algorithm）+ 参数（颜色/线宽/线型）”的方式进行交互。
...
- 右侧**算法（Algorithm）**下拉框：随模式动态重建，确保“文本项 ↔ 稳定算法ID”一致。切换模式时会自动刷新并写回 `dlg.Algorithm`。  
- 线宽输入框（`LineWidth`）、线型单选（实线/虚线）、两枚**取色按钮**：分别设置画线颜色 `LineColor` 与填充颜色 `ShapeColor`。  
- **填充（Filled/Not Filled）**：控制多边形/椭圆是否填充。  
...
- Fill：Scanline Fill（占位 ID 9 / 实际填充实现为 `ALG_FILL_SCANLINE`）。  
...
### 3.4 圆弧（Arc）
1. 模式选 **Arc**，左键按下设置起点、移动设置终点；**角度**在 `Edit2` 输入（1~360°）。  
...
> 注：非均匀缩放会把“圆弧”形变为“椭圆弧”；对**非正圆的椭圆**进行旋转时，程序会将其**转换为多边形近似**再旋转，属于设计上的可视化近似。
...
- 模式选 **Bezier**：  
  1) **左键单击**依次添加控制点；若命中已有控制点则进入**拖动修改**。  
  2) 控制折线与点有预览；≥3点时同步预览曲线。  
  3) **按 Ctrl** 结束并生成曲线对象；之后可在 Select 模式选中并变换。

- 命中判定基于“采样折线的点到线段最短距离”，绘制尊重线宽/虚线；自带采样缓存提升效率。

### 6.2 B-样条（de Boor，挑战）
- 模式选 **B-Spline**：  
  1) **左键**逐点录入控制点；若命中已有控制点则拖动修改。  
  2) 当点数 ≥ `p+1`（默认 `p=3`）时显示曲线预览（自动生成**均匀夹持**节点向量）。  
  3) **按 Ctrl** 结束并生成曲线对象。
...
- 模式选 **Connect**，分两步：  
  1) 第一次点击：命中**第一个**图元的轮廓/顶点，程序以 XOR 方式标注锚点；  
  2) 第二次点击：命中**第二个**图元，程序将其**C0 平移对齐**到第一点上（零缝拼接）。
...
- **为什么有时还会看到轻微闪烁？**  
  已采用**双缓冲**与禁擦背景，通常能抑制。若你在**临时预览**中使用了 XOR 擦绘，局部仍会有轻微跳变，属正常。最终绘制会走一次性 BitBlt。  

- **椭圆旋转后为什么变多边形？**  
  非正圆旋转会产出**倾斜矩形**无法直接表示，程序将椭圆采样为多边形近似再旋转，属设计取舍。  

- **非均匀缩放下的圆弧变形？**  
  理论上圆弧经非均匀缩放会成为椭圆弧，而当前 `ArcObject` 不携带椭圆参数，故仅对端点做几何变换，视觉近似即可。
...
- 选择模式：`Mode = Select` → 点击边界可选中，Shift 多选，Esc 取消。  
- 参考点：Select 模式**右键**切换“重心/自定义”。  
- 变换：Select 模式下拉选 **Translation/Scaling/Rotation** → 拖动。Scaling 时按 **Shift** 做各向异性缩放。  
- 多边形/Bezier/B-样条：**Ctrl** 结束输入。  
- 圆/弧：**Shift** 正圆/切换弧方向。  
- 连接：Mode=Connect → 依次点中两个图元轮廓，实现 C0 平移对接。
